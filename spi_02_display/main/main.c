...
// 🖼️ Fill screen with color using DMA buffer
void fill_color(uint16_t color) {
    set_window(0, 0, 239, 319); // Full screen

    static uint8_t row_buf[240 * 2];
    for (int i = 0; i < 240; i++) {
        row_buf[i * 2] = color >> 8;
...
// 🖥️ ILI9341 initialization sequence
void ili9341_init() {
    ...
    send_cmd(0x29); vTaskDelay(pdMS_TO_TICKS(120)); // Display on
}
...
void app_main(void) {
    ...
    ret = spi_bus_add_device(SPI2_HOST, &spi_device_config, &spi);
    if (ret != ESP_OK) {
